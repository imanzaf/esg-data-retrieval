...
2) Parse the relevant pages using LlamaParse, determined by mentions
   of Scope 1, Scope 2, years/FY, and units.
3) Output tables to markdown in:
   data/parsed_outputs/{company}_raw_parsed.md
...
Usage examples:
  python src/extract/llama_parse_markdown.py "NVIDIA CORP" \
    "https://images.nvidia.com/aem-dam/Solutions/documents/FY2024-\
NVIDIA-Corporate-Sustainability-Report.pdf"
  python src/extract/llama_parse_markdown.py "MICROSOFT CORP" \
    "https://query.prod.cms.rt.microsoft.com/cms/api/am/binary/RW1lmju"
  python src/extract/llama_parse_markdown.py "AMAZON COM INC" \
    "https://sustainability.aboutamazon.com/2023-amazon-\
sustainability-report.pdf"
  python src/extract/llama_parse_markdown.py "META PLATFORMS INC CLASS A" \
    "https://sustainability.atmeta.com/wp-content/uploads/2024/08/\
Meta-2024-Sustainability-Report.pdf"
  python src/extract/llama_parse_markdown.py "ALPHABET INC CLASS A" \
    "https://www.smartenergydecisions.com/upload/research_+_reports/\
google-2024-environmental-report.pdf"
  python src/extract/llama_parse_markdown.py "APPLE INC" \
...
class EmissionsDataExtractor:
        ...
        if self.openai_api_key:
            ...
            if final_json:
                ...
                with open(json_file, "w", encoding="utf-8") as jf:
                    jf.write(final_json)
                logger.info(f"Final JSON saved: {json_file}")
    ...
    def send_to_openai(self, raw_markdown: str) -> str | None:
        """
        Sends the parsed markdown to OpenAI with a prompt to produce
        the final JSON structure.
        """
        if not self.openai_api_key:
            logger.error("Missing OPENAI_API_KEY. Cannot proceed with LLM.")
            return None

        client = OpenAI(api_key=self.openai_api_key)
        prompt_instructions = (
            f"You are given sustainability report tables as markdown below."
            f"{raw_markdown}\n\n"
            "1. Analyze all tables and identify the one with the most complete "
            "year-by-year data for each scope (Scope 1, Scope 2 market-based, "
            "Scope 2 location-based).\n"
            "2. Use the table data to populate the JSON structure below. For "
            "Scope 2, include both market-based and location-based data if "
            "available, and explicitly note which is which.\n"
            "3. Convert all values to tCO2e, keeping units consistent across "
            "years.\n"
            "4. Use null for years where data is missing or unavailable.\n"
            "5. Return only the JSON structure below, with no explanations or "
            "additional text:\n\n"
            "{\n"
            ...
            "accurate data across all years. Prioritize completeness and "
            "consistency.\n"
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 3:
        print(
            "Usage: python src/extract/llama_parse_markdown.py <company_name> "
