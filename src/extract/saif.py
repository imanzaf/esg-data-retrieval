...
for filename in os.listdir(directory_path):
    if filename.endswith(".csv"):
        ...
        try:
            ...
            date_columns = [
                col for col in df.columns if re.search(regex_date, str(col))
            ]
            if not date_columns:
                print(
                    f"Skipping file {filename} as it has no date-related columns in the header."
                )
...
if scope_data:
    ...
    date_columns = [
        col for col in combined_scope_data.columns if re.search(regex_date, str(col))
    ]
