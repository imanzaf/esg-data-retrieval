...
def standardize_emissions_table(raw_data):
    ...
    raw_data.columns = raw_data.columns.str.strip().str.replace(
        "Unnamed.*", "", regex=True
    )

    # Normalize column names to detect financial years
    raw_data.columns = raw_data.columns.str.lower().str.replace(
        "[^a-z0-9 ]", "", regex=True
    )
    ...
    financial_years = [
        ...
        if pd.Series(col).str.contains(r"\b\d{4}\b").any()
    ]
    ...
    for idx, col in enumerate(raw_data.columns):
        if (
            raw_data[col]
            .astype(str)
            .str.contains(r"scope\s*1", case=False, regex=True)
            .any()
        ):
    ...
    scope_1_row_index = (
        raw_data.iloc[:, scope_1_column_index]
        .astype(str)
        .str.contains(r"scope\s*1", case=False, regex=True)
        .idxmax()
    )
...
if __name__ == "__main__":
    ...
    raw_data = pd.read_csv(
        os.path.join(OUTPUT_DIR, "GOLDMAN_SACHS_GROUP_INC", "esg_data.csv")
    )
