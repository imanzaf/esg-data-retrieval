...
def get_emissions_data_pro(identifier, idType, parser):
    ...
    if os.path.isfile(llm_output_file):
        ...
        pdf_files = [
            f
            for f in os.listdir(esg_reports.output_path)
        ...
        if pdf_files:
            ...
            logger.warning(
                f"No PDF found in {esg_reports.output_path} for cached data."
            )
    ...
    for url in esg_reports.urls.values():
        ...
        try:
            ...
            output = TableExtractor(
                company, local_pdf_path, parser, esg_reports.output_path
            ).extract()
