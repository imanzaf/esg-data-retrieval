...
def get_data():
    # Load the CSV file
    csv_file_path = os.path.join(ROOT_DIR, 'data/XWD_holdings.csv')  # Path to your input CSV file
    df = pd.read_csv(csv_file_path, header=0)  # Assuming header is already in the first row (change as needed)
...
if __name__ == '__main__':
    ...
    filtered_df = filter_dataframe(get_data(), country=country_input, sector=sector_input)
